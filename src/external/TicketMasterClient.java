...
public class TicketMasterClient {
	...
	private static final String API_KEY = "6nQxN8ZH40WlDs3OAnh27FVrYe0AH2Kv";
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			...
			// Get the **status code** from an HTTP response message. To execute the request
			// we can use the getResponseCode(), connect(), getInputStream() or
			...
			// get string from response，since we are client，TicketMaster is server
			// 分好几段返回，read by InputStreamReader
		...
		try {
			...
			// obj: _links _embedded page (only these 3 keys)
	...
	// Purify method to convert JSONArray to a list of item objects.
	private List<Item> getItemList(JSONArray events) throws JSONException {
		...
		for (int i = 0; i < events.length(); ++i) {
			...
			// 层级不是在最上一层，不能一步直接调用 ==> helper methods
